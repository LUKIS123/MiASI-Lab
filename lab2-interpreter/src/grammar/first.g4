...
stat: expr SEMI #expr_stat
    | 'int' varName=ID ('=' varVal=expr)? SEMI #var_def
    | IF_kw '(' cond=expr ')' then=block  ('else' else=block)? SEMI? #if_stat
    | WHILE_kw '(' cond=expr ')' loopBody=block SEMI? # whileLoop
    | '$' expr SEMI #print_stat
    ;
